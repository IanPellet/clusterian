...
def hierarchical_MC(diff, rope, rho,   upperAlpha=2, lowerAlpha =1, lowerBeta = 0.01, upperBeta = 0.1, std_upper_bound=1000, names=('C1', 'C2') ):
    ...
    */
    ...
    #Call to Stan code
    posterior = stan.build(program_code=hierarchical_code, data=hierachical_dat)
    fit = posterior.sample(num_chains=4, num_samples=1000)
    
    la = fit  # return a dictionary of arrays
    mu = la['delta0']
    stdh = la['std0']
    nu = la['nu']
...
def plot_simplex(points, names=('C1', 'C2')):
    ...
    for i, n in enumerate((nl, ne, nr)):
        if n < 0.001:
            ...
            coords = sorted(list(range(3)))
            coords.remove(i)
            #labels = np.array(['p({})'.format(names[0]), 'p(rope)', 'p({})'.format(names[1])])
            labels = np.array([0,1,2])
            fig.gca().add_line(Line2D(points[:, coords], labels[coords]))
            return fig
